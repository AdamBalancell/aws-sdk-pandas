...
def _to_orc(
    ...
    with pyorc.Writer(path, schema, compression=compression) as writer:
        for batch in _data_types.pandas_chunks(
            df=df, cpus=cpus, use_threads=use_threads, max_rows_by_file=max_rows_by_file
        ):
            batch = _data_types.pyarrow_table_from_pandas(df=batch, schema=schema, index=index, dtype=dtype)
            for record in batch:
                writer.write(record.to_pydict())
    return [path]
...
def to_orc(
    ...
    glue_table_settings: Optional[GlueTableSettings] = None,
    dtype: Optional[Dict[str, str]] = None,
    athena_partition_projection_settings: Optional[
        typing.AthenaPartitionProjectionSettings
    ] = None,
    catalog_id: Optional[str] = None,
) -> _S3WriteDataReturnValue:
